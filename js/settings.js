...
const FINTRACK_VERSION = 'v15.3.1';

function renderSettings(c) {
  if (window.innerWidth <= 768) { renderMobileSettings(c); return; }
  c.innerHTML = `<div class="stitle">${t('set_title')}</div><div class="ssub">${t('set_sub')}</div><div class="setg"><div class="setn"><div class="nsec">Profile</div><div class="sni active" onclick="setTab(this,'profile')"><i data-lucide="user" width="14" height="14"></i>Profile & Appearance</div><div class="nsec">Preferences</div><div class="sni" onclick="setTab(this,'currency')"><i data-lucide="coins" width="14" height="14"></i>${t('set_currency')}</div><div class="sni" onclick="setTab(this,'language')"><i data-lucide="languages" width="14" height="14"></i>${t('set_language')}</div><div class="nsec">Financial</div><div class="sni" onclick="setTab(this,'cataccounts')"><i data-lucide="layers" width="14" height="14"></i>Categories & Accounts</div><div class="sni" onclick="setTab(this,'general')"><i data-lucide="sliders" width="14" height="14"></i>${t('set_general')}</div><div class="nsec">System</div><div class="sni" onclick="setTab(this,'security')"><i data-lucide="shield" width="14" height="14"></i>${t('set_security')}</div></div><div class="setc" id="setc"></div></div>`;
  lucide.createIcons();
  setTab(null, 'profile');
}

// === MOBILE SETTINGS (v15.8.2 — Reorganized) ===
function renderMobileSettings(c) {
  const name = getUserName() || 'User';
  const initials = getUserInitials();
  c.innerHTML = `<div style="text-align:center;margin-bottom:20px"><div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,var(--accent),oklch(0.45 0.22 280));margin:0 auto 8px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#fff">${initials}</div><div style="font-size:15px;font-weight:700">${getUserTitle() ? getUserTitle().charAt(0).toUpperCase() + getUserTitle().slice(1) + ' ' : ''}${name}</div><div style="font-size:11px;color:var(--text-tertiary)">FinTrack ${FINTRACK_VERSION}</div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">Profile</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('profile')"><span class="mob-set-icon"><i data-lucide="user" width="18" height="18"></i></span><span class="mob-set-label">Profile & Appearance</span><span class="mob-set-val">&#8250;</span></div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">Preferences</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('currency')"><span class="mob-set-icon"><i data-lucide="coins" width="18" height="18"></i></span><span class="mob-set-label">Currency</span><span class="mob-set-val">${displayCurrency} &#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('language')"><span class="mob-set-icon"><i data-lucide="languages" width="18" height="18"></i></span><span class="mob-set-label">Language</span><span class="mob-set-val">${currentLang.toUpperCase()} &#8250;</span></div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">Financial</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('cataccounts')"><span class="mob-set-icon"><i data-lucide="layers" width="18" height="18"></i></span><span class="mob-set-label">Categories & Accounts</span><span class="mob-set-val">&#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('general')"><span class="mob-set-icon"><i data-lucide="sliders" width="18" height="18"></i></span><span class="mob-set-label">General</span><span class="mob-set-val">&#8250;</span></div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">System</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('security')"><span class="mob-set-icon"><i data-lucide="shield" width="18" height="18"></i></span><span class="mob-set-label">Security</span><span class="mob-set-val">${FT_APP_LOCK ? 'On' : 'Off'} &#8250;</span></div><div class="mob-set-item" onclick="checkForUpdates()"><span class="mob-set-icon"><i data-lucide="refresh-cw" width="18" height="18"></i></span><span class="mob-set-label">Check for Updates</span><span class="mob-set-val">&#8250;</span></div></div>`;
  lucide.createIcons();
}

function mobSetOpen(tab) {
  const c = document.getElementById('cnt');
  const tabNames = { profile: 'Profile & Appearance', general: 'General', currency: 'Currency', language: 'Language', cataccounts: 'Categories & Accounts', security: 'Security' };
