...
function mobSetOpen(tab) {
  ...
  let html = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px"><button class="btn bs" style="padding:6px 10px;font-size:11px" onclick="renderSettings(document.getElementById('cnt'))"><i data-lucide="arrow-left" width="12" height="12"></i> Back</button><span style="font-size:14px;font-weight:700">${tab === 'profile' ? 'Profile' : tab === 'general' ? 'Notifications' : tab === 'appearance' ? 'Appearance' : tab === 'currency' ? 'Currency' : tab === 'language' ? 'Language' : tab === 'cataccounts' ? 'Categories & Accounts' : 'Security'}</span></div><div id="setc"></div>`;
  c.innerHTML = html;
  lucide.createIcons();
