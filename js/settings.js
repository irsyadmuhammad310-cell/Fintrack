...
const FINTRACK_VERSION = 'v15.7.1';
...
function renderMobileSettings(c) {
  ...
  c.innerHTML = `<div style="text-align:center;margin-bottom:20px"><div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,var(--accent),oklch(0.45 0.22 280));margin:0 auto 8px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#fff">${initials}</div><div style="font-size:15px;font-weight:700">${getUserTitle() ? getUserTitle().charAt(0).toUpperCase() + getUserTitle().slice(1) + ' ' : ''}${name}</div><div style="font-size:11px;color:var(--text-tertiary)">FinTrack ${FINTRACK_VERSION}</div></div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('profile')"><span class="mob-set-icon"><i data-lucide="user" width="18" height="18"></i></span><span class="mob-set-label">Profile</span><span class="mob-set-val">&#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('appearance')"><span class="mob-set-icon"><i data-lucide="palette" width="18" height="18"></i></span><span class="mob-set-label">Appearance</span><span class="mob-set-val">${document.documentElement.dataset.theme === 'dark' ? 'Dark' : 'Light'} &#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('general')"><span class="mob-set-icon"><i data-lucide="sliders" width="18" height="18"></i></span><span class="mob-set-label">General</span><span class="mob-set-val">&#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('currency')"><span class="mob-set-icon"><i data-lucide="coins" width="18" height="18"></i></span><span class="mob-set-label">Currency</span><span class="mob-set-val">${displayCurrency} &#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('language')"><span class="mob-set-icon"><i data-lucide="languages" width="18" height="18"></i></span><span class="mob-set-label">Language</span><span class="mob-set-val">${currentLang.toUpperCase()} &#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('cataccounts')"><span class="mob-set-icon"><i data-lucide="layers" width="18" height="18"></i></span><span class="mob-set-label">Categories & Accounts</span><span class="mob-set-val">&#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('security')"><span class="mob-set-icon"><i data-lucide="shield" width="18" height="18"></i></span><span class="mob-set-label">Security</span><span class="mob-set-val">${FT_APP_LOCK ? 'On' : 'Off'} &#8250;</span></div></div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="checkForUpdates()"><span class="mob-set-icon"><i data-lucide="refresh-cw" width="18" height="18"></i></span><span class="mob-set-label">Check for Updates</span><span class="mob-set-val">&#8250;</span></div></div>`;
  lucide.createIcons();
}

function mobSetOpen(tab) {
  const c = document.getElementById('cnt');
  let html = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px"><button class="btn bs" style="padding:6px 10px;font-size:11px" onclick="renderSettings(document.getElementById('cnt'))"><i data-lucide="arrow-left" width="12" height="12"></i> Back</button><span style="font-size:14px;font-weight:700">${tab === 'profile' ? 'Profile' : tab === 'general' ? 'General' : tab === 'appearance' ? 'Appearance' : tab === 'currency' ? 'Currency' : tab === 'language' ? 'Language' : tab === 'cataccounts' ? 'Categories & Accounts' : 'Security'}</span></div><div id="setc"></div>`;
  c.innerHTML = html;
  lucide.createIcons();
