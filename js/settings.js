...
function renderSettings(c) {
  ...
  c.innerHTML = `<div class="stitle">${t('set_title')}</div><div class="ssub">${t('set_sub')}</div><div class="setg"><div class="setn"><div class="nsec">${t('set_profile')}</div><div class="sni active" onclick="setTab(this,'profile')"><i data-lucide="user" width="14" height="14"></i>${t('set_profile')} & ${t('set_appearance')}</div><div class="nsec">${t('set_currency')}</div><div class="sni" onclick="setTab(this,'currency')"><i data-lucide="coins" width="14" height="14"></i>${t('set_currency')}</div><div class="sni" onclick="setTab(this,'language')"><i data-lucide="languages" width="14" height="14"></i>${t('set_language')}</div><div class="nsec">${t('set_cat_accounts')}</div><div class="sni" onclick="setTab(this,'cataccounts')"><i data-lucide="layers" width="14" height="14"></i>${t('set_cat_accounts')}</div><div class="sni" onclick="setTab(this,'general')"><i data-lucide="sliders" width="14" height="14"></i>${t('set_general')}</div><div class="nsec">${t('set_security')}</div><div class="sni" onclick="setTab(this,'security')"><i data-lucide="shield" width="14" height="14"></i>${t('set_security')}</div></div><div class="setc" id="setc"></div></div>`;
  lucide.createIcons();
  setTab(null, 'profile');
}
...
function renderMobileSettings(c) {
  ...
  c.innerHTML = `<div style="text-align:center;margin-bottom:20px"><div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,var(--accent),oklch(0.45 0.22 280));margin:0 auto 8px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#fff">${initials}</div><div style="font-size:15px;font-weight:700">${getUserTitle() ? getUserTitle().charAt(0).toUpperCase() + getUserTitle().slice(1) + ' ' : ''}${name}</div><div style="font-size:11px;color:var(--text-tertiary)">FinTrack ${FINTRACK_VERSION}</div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">${t('set_profile')}</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('profile')"><span class="mob-set-icon"><i data-lucide="user" width="18" height="18"></i></span><span class="mob-set-label">${t('set_profile')} & ${t('set_appearance')}</span><span class="mob-set-val">&#8250;</span></div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">${t('set_currency')}</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('currency')"><span class="mob-set-icon"><i data-lucide="coins" width="18" height="18"></i></span><span class="mob-set-label">${t('set_currency')}</span><span class="mob-set-val">${displayCurrency} &#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('language')"><span class="mob-set-icon"><i data-lucide="languages" width="18" height="18"></i></span><span class="mob-set-label">${t('set_language')}</span><span class="mob-set-val">${currentLang.toUpperCase()} &#8250;</span></div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">${t('set_cat_accounts')}</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('cataccounts')"><span class="mob-set-icon"><i data-lucide="layers" width="18" height="18"></i></span><span class="mob-set-label">${t('set_cat_accounts')}</span><span class="mob-set-val">&#8250;</span></div><div class="mob-set-item" onclick="mobSetOpen('general')"><span class="mob-set-icon"><i data-lucide="sliders" width="18" height="18"></i></span><span class="mob-set-label">${t('set_general')}</span><span class="mob-set-val">&#8250;</span></div></div><div style="font-size:9px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.08em;padding:0 4px 6px">${t('set_security')}</div><div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:14px"><div class="mob-set-item" onclick="mobSetOpen('security')"><span class="mob-set-icon"><i data-lucide="shield" width="18" height="18"></i></span><span class="mob-set-label">${t('set_security')}</span><span class="mob-set-val">${FT_APP_LOCK ? 'On' : 'Off'} &#8250;</span></div><div class="mob-set-item" onclick="checkForUpdates()"><span class="mob-set-icon"><i data-lucide="refresh-cw" width="18" height="18"></i></span><span class="mob-set-label">${t('set_check_updates')}</span><span class="mob-set-val">&#8250;</span></div></div>`;
  lucide.createIcons();
}

function mobSetOpen(tab) {
  const c = document.getElementById('cnt');
  const tabNames = { profile: t('set_profile') + ' & ' + t('set_appearance'), general: t('set_general'), currency: t('set_currency'), language: t('set_language'), cataccounts: t('set_cat_accounts'), security: t('set_security') };
  let html = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px"><button class="btn bs" style="padding:6px 10px;font-size:11px" onclick="renderSettings(document.getElementById('cnt'))"><i data-lucide="arrow-left" width="12" height="12"></i> ${t('misc_back')}</button><span style="font-size:14px;font-weight:700">${tabNames[tab] || tab}</span></div><div id="setc"></div>`;
  c.innerHTML = html;
  lucide.createIcons();
...
function renderGeneralTab(c) {
  ...
  html += `<div style="border:1px solid var(--border);border-radius:12px;padding:16px 18px;margin-bottom:16px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:12px"><div style="width:32px;height:32px;border-radius:8px;background:var(--blue-light);color:var(--blue);display:flex;align-items:center;justify-content:center"><i data-lucide="hard-drive" width="15" height="15"></i></div><div><div style="font-size:13px;font-weight:600">Storage</div><div style="font-size:10px;color:var(--text-tertiary)">${(storageUsed/1024).toFixed(0)} KB of ~5 MB used (${storagePct}%)</div></div></div><div style="height:6px;background:var(--border);border-radius:3px;overflow:hidden"><div style="height:100%;width:${storagePct}%;background:${storageColor};border-radius:3px;transition:width 300ms"></div></div></div>`;
  // Notifications section
  html += `<div style="border:1px solid var(--border);border-radius:12px;padding:16px 18px;margin-bottom:16px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:14px"><div style="width:32px;height:32px;border-radius:8px;background:var(--blue-light);color:var(--blue);display:flex;align-items:center;justify-content:center"><i data-lucide="bell" width="15" height="15"></i></div><div><div style="font-size:13px;font-weight:600">${t('set_notifications')}</div><div style="font-size:10px;color:var(--text-tertiary)">${t('set_reminders')}</div></div></div><div class="trow"><div class="tinf"><div class="tna">${t('set_budget_alerts')}</div><div class="tde">${t('set_budget_alert_desc')}</div></div><div class="tsw ${localStorage.getItem('ft_budget_alerts') !== 'off' ? 'on' : ''}" onclick="this.classList.toggle('on');localStorage.setItem('ft_budget_alerts',this.classList.contains('on')?'on':'off')"></div></div><div class="trow"><div class="tinf"><div class="tna">${t('set_milestone_alerts')}</div><div class="tde">${t('set_milestone_desc')}</div></div><div class="tsw ${localStorage.getItem('ft_milestone_alerts') !== 'off' ? 'on' : ''}" onclick="this.classList.toggle('on');localStorage.setItem('ft_milestone_alerts',this.classList.contains('on')?'on':'off')"></div></div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:14px;margin-bottom:8px"><div style="font-size:11px;font-weight:600">${t('set_reminders')}</div><button class="btn bp" style="font-size:10px;padding:4px 10px" onclick="openReminderModal()"><i data-lucide="plus" width="10" height="10"></i> ${t('set_add_reminder')}</button></div>`;

  if (typeof REMINDERS !== 'undefined' && REMINDERS && REMINDERS.length) {
    ...
    html += `<div style="padding:16px;text-align:center;color:var(--text-tertiary);font-size:11px;border:1px dashed var(--border);border-radius:8px">${t('set_no_reminders')}</div>`;
  }
  html += `</div>`;
  // Years section
  html += `<div style="border:1px solid var(--border);border-radius:12px;padding:16px 18px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:14px"><div style="width:32px;height:32px;border-radius:8px;background:var(--emerald-light);color:var(--emerald);display:flex;align-items:center;justify-content:center"><i data-lucide="calendar" width="15" height="15"></i></div><div><div style="font-size:13px;font-weight:600">${t('set_year_mgmt')}</div><div style="font-size:10px;color:var(--text-tertiary)">${t('set_year_desc')}</div></div></div>`;
  html += `<div style="display:flex;gap:8px;margin-bottom:14px"><input class="fi" type="number" id="addYearInput" placeholder="e.g. 2041" style="max-width:120px;font-size:12px" min="1900" max="2100"><button class="btn bp" style="font-size:11px;padding:5px 12px" onclick="handleAddYear()"><i data-lucide="plus" width="11" height="11"></i> ${t('set_add_year')}</button></div>`;
  ...
  c.innerHTML = html;
  lucide.createIcons();
  } catch(e) { c.innerHTML = '<div style="padding:20px;color:var(--rose);font-size:12px"><b>Error loading General tab:</b><br>' + e.message + '</div>'; console.error('General tab error:', e); }
}
